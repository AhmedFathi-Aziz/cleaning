/**
 * يزيل الخلفية الرمادية من شعار الشركة ويولّد ملفات للهيدر والأيقونات.
...
async function main() {
  ...
  const pngOut = path.join(publicDir, "saudi-emblem.png");
  const webp192 = path.join(publicDir, "saudi-emblem-192.webp");
  const webp512 = path.join(publicDir, "saudi-emblem-512.webp");
  const faviconSrc = path.join(publicDir, "favicon.png");

  await transparent.clone().png({ compressionLevel: 9 }).toFile(pngOut);
